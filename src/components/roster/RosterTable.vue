...
const roster = computed<Roster | undefined>(() =>
   rosterStore.getRoster(props.id)
);
            ...
            if (newRoster.rosterShift) {
                for (const shift of newRoster.rosterShift) {
                    const value = existingAnswers.find(
                        (answer: RosterAnswer) =>
                            answer.rosterShiftId === shift.id && answer.userId === user.id
                    );
...
<template>
    <div v-if="roster && Object.keys(shiftAnswers).length" class="flex flex-row gap-2 w-full ">
        <div class="max-w-5xl overflow-x-auto">
            <table class="table-auto w-full min-w-full text-xs sm:text-sm lg:text-base">
                ...
                <th class="user-column text-[10px] sm:text-sm lg:text-base px-1 py-0.5">Users</th>
                    <th v-if="roster.rosterShift" v-for="shift in roster.rosterShift"
                        :key="shift.id"
        ...
        <div class="flex flex-col gap-2 justify-center">
            <Button label="Add Shift" @click="visible = true" :disabled="roster.saved" class="text-xs sm:text-sm lg:text-base px-2 sm:px-4 py-1 sm:py-2">
                Add Shift
            </Button>
            <Button label="Save Roster" @click="saveRoster" :disabled="roster.saved" class="text-xs sm:text-sm lg:text-base px-2 sm:px-4 py-1 sm:py-2">
                Save Roster
            </Button>
        </div>

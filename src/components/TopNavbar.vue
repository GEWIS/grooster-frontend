...
<template>
  <nav class="relative bg-emerald-950">
    <div class="flex items-center justify-between px-4 py-2">
        ...
        <div class="hidden md:flex items-center gap-1">
          <template v-if="route.path !== '/' && organStore.organ">
            ...
            <RouterLink
              :class="[
                'flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition-all duration-200',
                isActive('/templates/' + organStore.organ)
                  ? 'bg-emerald-800 text-white shadow-sm'
                  : 'text-emerald-100 hover:bg-emerald-900 hover:text-white',
              ]"
              :to="`/templates/${organStore.organ}`"
            >
      ...
      <div class="hidden lg:flex absolute left-1/2 -translate-x-1/2 items-center gap-2">
        <span class="text-xs uppercase tracking-widest text-emerald-500 font-bold">Organ:</span>
        <h1 class="text-white font-semibold">{{ organStore.organName ?? 'Not Selected' }}</h1>
      </div>
          ...
          <template v-if="route.path !== '/' && organStore.organ">
            <RouterLink
              ...
              :class="[
                ...
                isActive(`/organ/${organStore.organ}/profile`)
                  ? 'bg-emerald-800/50 border-emerald-700 text-white'
                  : 'text-emerald-100 hover:bg-emerald-900',
              ]"
              :to="`/organ/${organStore.organ}/profile`"
            >
              <span class="text-sm font-medium">{{ organStore.organ }}</span>
              <div
    ...
    >
      <template v-if="route.path !== '/' && organStore.organ">
        ...
        <RouterLink class="text-emerald-100 py-2" :to="`/rosters/${organStore.organ}`" @click="isMobileMenuOpen = false"
          >Rosters</RouterLink
        >
        <RouterLink

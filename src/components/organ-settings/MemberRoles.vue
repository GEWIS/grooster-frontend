      ...
      <div class="w-1/3 border-r border-emerald-50 overflow-y-auto bg-emerald-50/20">
        <div
          ...
          :class="[
            ...
            selectedUser?.id === user.id ? 'bg-emerald-100/50 border-r-4 border-r-emerald-600' : 'hover:bg-emerald-50',
          ]"
          @click="() => setUser(user)"
        >
          <div
            class="h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0"
            :class="selectedUser?.id === user.id ? 'bg-emerald-600 text-white' : 'bg-emerald-200 text-emerald-800'"
          >
            {{ user.name.charAt(0) }}
          </div>
          <p
            :class="['truncate text-sm', selectedUser?.id === user.id ? 'font-bold text-emerald-900' : 'text-gray-600']"
          >
            {{ user.name }}
          </p>
        </div>
      ...
      <div class="w-2/3 overflow-y-auto bg-white relative">
        <div v-if="selectedUser" class="p-8 h-full flex flex-col">
          ...
          <div class="space-y-6 bg-emerald-50/30 p-6 rounded-2xl border border-emerald-100">
            <div>
              <label class="block text-[10px] font-bold text-emerald-800 uppercase tracking-widest mb-2 px-1"

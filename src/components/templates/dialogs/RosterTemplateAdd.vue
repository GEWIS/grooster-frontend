    ...
    <Dialog v-model:visible="visible" modal :style="{ width: '28rem' }" @hide="emit('close')">
        ...
        <div class="flex flex-col gap-6 pt-2">
            <div class="flex flex-col gap-2">
                <label for="templateName" class="font-bold text-sm text-surface-700">Template Name</label>
                <InputText id="templateName" v-model="templateName" placeholder="e.g. Weekly Operations" class="w-full" />
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <Form v-slot="$form" :initial-values :resolver @submit="onFormSubmit" class="flex gap-2">
                    <div class="flex-1">
                        <InputText
                            ref="shiftNameInput"
                            name="shiftName"
                            ...
                            type="text"
                        />
                        <Message v-if="$form.shiftName?.invalid" severity="error" size="small" variant="simple" class="mt-1">
                            {{ $form.shiftName.error?.message }}
                        </Message>
                    </div>
                    <Button label="Add" icon="pi pi-plus" severity="primary" type="submit" />
                </Form>
            </div>
